...
MODULE mod_h5_utility
    ...
    TYPE h5_dataset_type
        ...
        LOGICAL           :: is_created = .FALSE., is_destroyed = .TRUE. ! These ensure that create and destructor subroutines are executed only once.
...
WRITE(error_unit,*) "mod_h5_utility/create/{"
            ...
            CALL separate_string(data_address, address_parts) ! eg., data_address = directory/address/h5file.h5/group1/group2 -> adress_parts(1) = file name = directory/address/h5file.h5, and subsequent parts are considered as group names. 
...
WRITE(error_unit,*) "mod_h5_utility/h5_utility_set_datatype}"
        ...
        !###########################################################
        SUBROUTINE separate_string(textin, out_text) ! eg., data_address = directory/address/h5file.h5/group1/group2 -> adress_parts(1) = file name = directory/address/h5file.h5, and subsequent parts are considered as group names.
            CHARACTER(LEN=*), INTENT(IN) :: textin
            CHARACTER(LEN=:), DIMENSION(:), ALLOCATABLE, INTENT(OUT) :: out_text
            CHARACTER(LEN=:), ALLOCATABLE :: text, file_address
...
WRITE(error_unit,*) "mod_h5_utility/separate_string/{"
            ...
            ! Separating file address
            i = INDEX(textin, '.h5')
            IF (i == 0) THEN
                WRITE(error_unit,*) 'ERROR: No .h5 extension found in the h5 file name: ', TRIM(textin)
                ERROR STOP
            END IF
            file_address = textin(1:i+2) ! file address
            text = textin(i:) ! contains groups, eg., .h5/group1/group2/, in the following separation is using '/', hence .h5 is kept here.
