...
4. With restart option, restart index is required from where the date is resumed to be written. Note that the data is written exactly from the restart index if the restart index has same iteration value as the restart iteration value. Therefore the data corresponding to that iteration has to be written first before proceeding with further computation in such cases.
