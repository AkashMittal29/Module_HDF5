...
PROGRAM main
    ...
    INTEGER(kind=HSIZE_T) :: file_slab_start(2), file_slab_end(2), file_slab_count(2) 
    ...
    INTEGER(kind=HSIZE_T) :: dims_current(2), dims_chunk(2), dims_mem(2)
    ...
    dims_current = (/7,5/)
    CALL h5screate_simple_f(2, dims_current, space_id, error)
       
    ! Creating property list for chunking
    dims_chunk = dims_current
    CALL h5pcreate_f(H5P_DATASET_CREATE_F, plist_id, error)
    CALL h5pset_chunk_f(plist_id, 2, dims_chunk, error) 
