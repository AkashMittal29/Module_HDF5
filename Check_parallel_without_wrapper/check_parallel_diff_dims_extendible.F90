...
PROGRAM main
    ...
    INTEGER(kind=hid_t)   :: file_id, space_id, dataset_id(2), plist_id, memspace_id, transfer_plist_id
    INTEGER(kind=HSIZE_T) :: dims_current(3), dims_chunk(3), dims_mem(2), dims_max(3)
    ...
    dims_current = (/7,5,0/)
    dims_max = dims_current; dims_max(3) = HUGE(0_HSIZE_T)
    CALL h5screate_simple_f(3, dims_current, space_id, error, dims_max)
    ...
    ! Creating dataset id
    CALL h5dcreate_f(file_id, 'x', H5T_NATIVE_DOUBLE, space_id, dataset_id(1), error, plist_id) 

    !#TODO: check with array of dataset
    ! Extending last dimension
    dims_current(3) = 1
    CALL h5dset_extent_f(dataset_id(1), dims_current, error)
    ...
    ! Writing data
    CALL h5dwrite_f(dataset_id(1), H5T_NATIVE_DOUBLE, x, dims_mem, &
    ...
    CALL h5dclose_f(dataset_id(1), error)
